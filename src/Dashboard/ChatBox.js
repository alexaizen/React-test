    ...
    const pushMessage = function (message) {
      fetch(
        "https://react-1bbaa-default-rtdb.europe-west1.firebasedatabase.app/messages.json",
        {
          method: "POST",
          body: JSON.stringify({
            idMsg: `Message user ${props.user.id}`,
        ...
        if (res.ok) {
          ...
          return res.json().then((data) => console.log("error" + data)).catch();
        }
      }).then(()=>{
        fetchMessages()
      }).catch();
      ...
      if (res.ok) {
        ...
        return res.json().then((data) => {
          for (const key in data) {
            ...
            // }
            messageArrayUpdate({idMsg: data[key].idMsg,
    ...
    <React.Fragment>
      <div className="message-area">
        <div className="message-area-inner">
          {messagesArray.map((msg)=> <Message avatar={msg.avatarMsg} name={msg.nameMsg} message={msg.messageMsg}></Message>)}
        </div>
      </div>
      <form onSubmit={sendMessage}>
        ...
        <button type="button" onClick={fetchMessages}>Get Msg</button>
      </form>
    </React.Fragment>
  );
}
